...
default_custom_colors = {
        ...
        "element_background_color" : '#1E1E1E', 
        "scrollbar_color" : '#353535', 
        "input_elements_background_color" : '#353535',
        "button_color" : ['#FDFDFD', '#353535'], 
        "text_color" : "#FDFDFD",
...
def color_load():
    ...
    try:
        ...
        color_options = {}
        with open("colorconfig.txt", "r") as file:
            for line in file:
                key, value = line.strip().split(":", maxsplit=1)
                value = json.loads(value)
                color_options[key] = value

            if color_options["enabled"] == True:
            
                psg.SetOptions(
                
                background_color = color_options["background_color"], 
                text_element_background_color = color_options["text_element_background_color"],
                element_background_color = color_options["element_background_color"], 
                scrollbar_color = color_options["scrollbar_color"], 
                input_elements_background_color = color_options["input_elements_background_color"],
                button_color = color_options["button_color"]
                
                ) 

                psg.theme_text_color(color_options["text_color"])
                psg.theme_input_text_color(color_options["input_text_color"])
...
column_two = [
             ...
             [psg.Push(), psg.Text("Diva Mod Enabler V1.5", size = 20), psg.Text("Mods: " + str(len(mod_folders)), size = 8, key = "--Mod Count--")]
             ]
column_three = [
...
layout = [
         [psg.Column(column_one), psg.VSeparator(), psg.Column(column_two), psg.Column(column_three, visible = False, key = "--Options--")],
         ]
...
#add options menu here
    ...
    if event == "Download Mods":
        wb.open("https://gamebanana.com/games/16522", 2)

    if event == "Export Mods List":
        ...
        with open("Modlist.txt", "w") as file:
            file.write(mod_list)
    ...
    if event == "Generate Color Config":
        ...
        with open("colorconfig.txt", "w") as f:
            for key, value in default_custom_colors.items():
                f.write(f"{key}: {json.dumps(value)}\n")


        psg.popup_auto_close("Created colorconfig.txt at\n" + os.getcwd(), auto_close_duration = 2, title = "DivaModEnabler")
        window["--Text--"].update("Log: Created Color Config")
